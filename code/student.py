...
def my_imfilter(image, kernel):
    ...
    # Your code here #
    print('my_imfilter function is started')
    kernel_row = kernel.shape[0]
    kernel_col = kernel.shape[1]

    assert kernel_row % 2 == 1
    assert kernel_col % 2 == 1

    img_row = image.shape[0]
    img_col = image.shape[1]
    img_channel = image.shape[2] if len(image.shape) == 3 else 1

    stride = 1

    # img_new_shape = (img_old_shape - kernel_shape) / stride + 1
    # => Padding shape mean img new shape == old_shape => Padding size

    padding_row = int((img_row * (stride - 1) + kernel_row - 1) / 2)
    padding_col = int((img_col * (stride - 1) + kernel_col - 1) / 2)

    padding_shape = ((padding_row,), (padding_col,)) if img_channel == 1 \
        else ((padding_row,), (padding_col,), (0,))
    padding_image = np.pad(image,
                           padding_shape,
                           mode='constant',
                           constant_values=0)
    ...
    for row in range(0, img_row):
        for col in range(0, img_col):
            filtered_image[row, col] = np.sum(np.sum(np.multiply(
                padding_image[row: row + kernel_row, col: col + kernel_col],
                kernel), axis=1), axis=0)
    filtered_image = normalize(filtered_image)
    print('my_imfilter function is ended')
    ##################
